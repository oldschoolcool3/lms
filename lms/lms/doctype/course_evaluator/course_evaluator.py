...
class CourseEvaluator(Document):
    ...
    if TYPE_CHECKING:
        ...
        from lms.lms.doctype.evaluator_schedule.evaluator_schedule import EvaluatorSchedule

        evaluator: DF.Link
        full_name: DF.Data | None
        schedule: DF.Table[EvaluatorSchedule]
        unavailable_from: DF.Date | None
        unavailable_to: DF.Date | None
    ...
    def validate_overlaps(self, schedule):
        ...
        for slot in same_day_slots:
            ...
            if get_time(schedule.start_time) < get_time(slot.end_time) <= get_time(schedule.end_time):
                overlap = True
            if get_time(slot.start_time) < get_time(schedule.start_time) and get_time(schedule.end_time) < get_time(
                slot.end_time
            ):
                overlap = True
...
def get_evaluator_availability(evaluator):
    return frappe.db.get_value("Course Evaluator", evaluator, ["unavailable_from", "unavailable_to"], as_dict=1)
