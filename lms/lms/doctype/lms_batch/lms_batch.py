    ...
    def validate_payments_app(self):
        if self.paid_batch:
            ...
            if "payments" not in installed_apps:
                ...
                frappe.throw(
                    _(
                        "Please install the Payments App to create a paid batch. Refer to the documentation for more details. {0}"  # noqa: E501 — translatable message; keep msgid on one line
        ...
        for schedule in self.timetable:
            if schedule.start_time and schedule.end_time:
                if get_time(schedule.start_time) > get_time(schedule.end_time) or get_time(
                    ...
                    frappe.throw(
                        _("Row #{0} Start time cannot be greater than or equal to end time.").format(schedule.idx)
                    )

                if get_time(schedule.start_time) < get_time(self.start_time) or get_time(
                    schedule.start_time
                ) > get_time(self.end_time):
                    frappe.throw(_("Row #{0} Start time cannot be outside the batch duration.").format(schedule.idx))

                if get_time(schedule.end_time) < get_time(self.start_time) or get_time(schedule.end_time) > get_time(
                    self.end_time
                ):
    ...
    def validate_conferencing_provider(self):
        ...
        if self.conferencing_provider == "Google Meet":
            ...
            if not google_meet_settings.enabled:
                frappe.throw(
                    _("The selected Google Meet account is disabled. Please enable it or select another account.")
                )
    ...
    notification = frappe._dict(
        {
            ...
            "email_content": _("A new batch '{0}' has been published that might interest you. Check it out!").format(
                batch.title
            ),
...
def create_google_meet_live_class(
    ...
    if not google_meet_settings.google_calendar:
        frappe.throw(
            _(
                "The Google Meet account does not have a Google Calendar configured. Please set up a Google Calendar first."  # noqa: E501 — translatable message; keep msgid on one line
            )
        )
...
def authenticate(zoom_account):
    ...
    response = requests.request("POST", authenticate_url, headers=headers, timeout=30)
