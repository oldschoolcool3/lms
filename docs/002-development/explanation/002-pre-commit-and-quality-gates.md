...
- ESLint grandfathers existing violations in `frontend/eslint-suppressions.json` (ESLint's native bulk-suppressions) and fails only on **new** ones. This was burned down from ~480 to **zero**: the final residue -- `vue/no-mutating-props` (plus one coupled `vue/no-dupe-keys`) across four form components (CouponDetails/SettingFields/EditProfile/VideoBlock) that edited a passed-in reactive object in place -- was cleared by a deliberate prop -> `defineModel`/`v-model` refactor, so the ledger is now empty (`{}`). Any new violation fails the gate until it is fixed or re-suppressed.
