...
def _resolve_lesson_references(file_url: str) -> list[tuple[str, bool]]:
    ...
    for r in frappe.db.get_all(
        ...
        if r.attached_to_name:
            refs.append((r.attached_to_name, r.attached_to_field in INSTRUCTOR_FIELDS or not r.attached_to_field))
...
# Guest-safe (reviewed): path-validated; access-gated.
...
def serve_resource(file_url: str):
    ...
    if not any(can_access_lesson(lesson, instructor_only=instructor_only) for lesson, instructor_only in references):
...
def save_progress(lesson: str, course: str, scorm_details: dict = None):
    ...
    progress_already_exists = frappe.db.exists("LMS Course Progress", {"lesson": lesson, "member": frappe.session.user})
...
def get_next_lesson(course: str, lesson: str):
    lesson_reference = frappe.db.get_value("Lesson Reference", {"lesson": lesson}, ["idx", "parent"], as_dict=1)
