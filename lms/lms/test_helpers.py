...
class BaseTestUtils(UnitTestCase):
    ...
    def _create_lesson(self, title, chapter, course, content=None):
        ...
        if not content:
            content = '{"time":1765194986690,"blocks":[{"id":"dkLzbW14ds","type":"markdown","data":{"text":"This is a simple content for the current lesson."}},{"id":"KBwuWPc8rV","type":"markdown","data":{"text":""}}],"version":"2.29.0"}'  # noqa: E501 — inline JSON fixture content
    ...
    def _setup_lessons(self):
        for index, chapter_ref in enumerate(self.course.chapters):
            ...
            for j in range(1, 5):
                ...
                if not any(existing.lesson == lesson.name for existing in chapter_doc.lessons):
    ...
    def _create_lesson_progress(self, member, course, lesson):
        existing = frappe.db.exists("LMS Course Progress", {"member": member, "course": course, "lesson": lesson})
        if existing:
    ...
    def _create_quiz_submission(self, member):
        ...
        submission = frappe.db.get_value("LMS Quiz Submission", {"quiz": self.quiz.name, "member": member}, "name")
        self.cleanup_items.append(("LMS Quiz Submission", submission))
        frappe.session.user = "Administrator"

    def _create_assignment_submission(self, member):
        existing = frappe.db.exists("LMS Assignment Submission", {"assignment": self.assignment.name, "member": member})
        if existing:
