...
class LMSCertificate(Document):
    ...
    def validate_batch_enrollment(self):
        if self.batch_name:
            is_enrolled = frappe.db.exists("LMS Batch Enrollment", {"batch": self.batch_name, "member": self.member})
            if not is_enrolled:
    ...
    def validate_course_enrollment(self):
        if self.course:
            ...
            if not is_enrolled:
                frappe.throw(_("Certification cannot be issued as the member is not enrolled in this course."))
            ...
            if completion_certificate:
                ...
                if progress < 100:
                    frappe.throw(_("Certification cannot be issued as the member has not completed the course."))
    ...
    def validate_batch_duplicates(self):
        if self.batch_name:
            ...
            if len(batch_duplicates):
                full_name = frappe.db.get_value("User", self.member, "full_name")
                frappe.throw(
                    _("{0} is already certified for the batch {1}").format(full_name, batch_duplicates[0].batch_title)
                )
...
def create_certificate(course: str):
    ...
    if certificate:
        return frappe.db.get_value("LMS Certificate", certificate, ["name", "course", "template"], as_dict=True)
...
def validate_certification_eligibility(course):
    ...
    progress = frappe.db.get_value("LMS Enrollment", {"course": course, "member": frappe.session.user}, "progress")
    if progress < 100:
