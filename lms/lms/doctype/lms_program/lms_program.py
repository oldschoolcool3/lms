...
class LMSProgram(Document):
    ...
    def validate_program_courses(self):
        ...
        if len(duplicates):
            frappe.throw(
                _("Course {0} has already been added to this program.").format(frappe.bold(next(iter(duplicates))))
            )
    ...
    def validate_program_members(self):
        ...
        if len(duplicates):
            frappe.throw(
                _("Member {0} has already been added to this program.").format(frappe.bold(next(iter(duplicates))))
            )
