...
class LMSQuiz(Document):
    ...
    def calculate_total_marks(self):
        ...
        if self.limit_questions_to:
            self.total_marks = sum(question.marks for question in self.questions[: cint(self.limit_questions_to)])
        else:
    ...
    def validate_open_ended_questions(self):
        ...
        if "Open Ended" in types:
            if len(types) > 1:
                frappe.throw(
                    _(
                        "If you want open ended questions then make sure each question in the quiz is of open ended type."  # noqa: E501 — translatable message; keep msgid on one line
                    )
                )
            else:
                self.show_answers = 0
...
def submit_quiz(quiz: str, results: "str | list | None" = None):
    ...
    submission = create_submission(quiz, data["results"], quiz_details.total_marks, quiz_details.passing_percentage)
...
def process_results(results: list, quiz_details: "frappe._dict"):
    ...
    for result in results:
        ...
        if question_details.type != "Open Ended":
            ...
            is_open_ended = True
            result["is_correct"] = 0
            result["answer"] = re.sub(r'<img[^>]*src\s*=\s*["\'](?=data:)(.*?)["\']', _save_file, result["answer"][0])
...
def verify_answer(question: str, answer: list):
    ...
    if question_details.multiple:
        for option_field, correctness_field in zip(QUESTION_OPTION_FIELDS, QUESTION_CORRECTNESS_FIELDS, strict=True):
