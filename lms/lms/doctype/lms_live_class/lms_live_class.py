...
class LMSLiveClass(Document):
    ...
    def build_event_description(self):
        description = (
            f"A Live Class has been scheduled on {format_date(self.date, 'medium')} "
            f"at {format_time(self.time, 'hh:mm a')}."
        )
        if self.join_url:
    ...
    def create_calendar_event(self):
        if self.conferencing_provider == "Google Meet":
            calendar = frappe.db.get_value("LMS Google Meet Settings", self.google_meet_account, "google_calendar")
        else:
            calendar = frappe.db.get_value("Google Calendar", {"user": frappe.session.user, "enable": 1}, "name")

        if not calendar:
            frappe.throw(
                _("No calendar is configured for the conferencing provider. Please set up a calendar to create events.")
            )
...
def get_attendance(live_class):
    ...
    if response.status_code != 200:
        frappe.throw(
            _("Failed to fetch attendance data from Zoom for class {0}: {1}").format(live_class, response.text)
        )
